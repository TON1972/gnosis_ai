...
export function StatsOverview({ stats }: { stats: any }) {
  ...
  const cards = [
    ...
    { 
      ...
      subValue: `${100 - Number(conversionRate)}% do ecossistema`,
      icon: UserPlus, 
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {cards.map((card, i) => (
          <Card key={i} className={`group p-6 bg-white border-l-10 ${card.color} shadow-sm hover:shadow-xl transition-all duration-300`}>
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                {/* Título: Menor, uppercase e bem espaçado */}
                <p className="text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.2em]">
                  {card.title}
                </p>
                
                <div className="flex items-baseline gap-1">
                  {/* Valor: Grande e pesado */}
                  <p className="text-5xl font-black text-[#1e3a5f] tracking-tighter">
                    {card.value?.toLocaleString('pt-BR') || 0}
                  </p>
                  {card.isPremium && (
                    <ArrowUpRight className="w-5 h-5 text-green-500 font-bold" />
                  )}
                </div>

                {/* Sub-título: Fonte média e cor suave */}
                <p className="text-sm font-semibold text-gray-500/80">
                  {card.subValue}
                </p>
              </div>

              <div className={`p-4 ${card.iconBg} rounded-2xl group-hover:rotate-6 transition-transform duration-300`}>
            ...
            <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
              <span className="text-[11px] font-bold text-gray-300 uppercase tracking-widest">Performance</span>
              <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase ${
                card.textColor === 'text-green-600' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
              }`}>
                {card.trend}
              </span>
            </div>
