...
const assertApiKey = () => {
  // Tenta pegar a chave da Forge ou da OpenAI como fallback
  if (!ENV.forgeApiKey && !process.env.OPENAI_API_KEY) {
    throw new Error("API Key (Forge ou OpenAI) não configurada no .env");
  }
};
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  // 3. Chamada para a API Oficial da OpenAI
  const url = "https://api.openai.com/v1/chat/completions";
  console.log("🚀 [LLM] Enviando requisição para OpenAI...");
  ...
  if (!response.ok) {
    ...
    console.error("❌ [LLM] Erro da OpenAI:", errorData);
    throw new Error(
